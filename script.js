// Supabase配置
const SUPABASE_URL = 'https://bvwskizieobuhfldonkl.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ2d3NraXppZW9idWhmbGRvbmtsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTk2NzA4OTgsImV4cCI6MjA3NTI0Njg5OH0.8-O3zKNcHazLycj1nzP1m7wHHcePmmT-m0K04FWSwZA';
...
function checkNetworkStatus() {
    ...
    // 测试网络连接
    return fetch('https://bvwskizieobuhfldonkl.supabase.co/rest/v1/', {
        method: 'HEAD',
        mode: 'no-cors'
    }).then(() => true).catch(() => {
...
function initSlideshow() {
    const slides = [
        { 
            src: 'pineapple1.png', 
            title: '凤梨种植基地',
            description: '位于揭阳市的优质凤梨种植基地，拥有得天独厚的自然条件'
        },
        { 
            src: 'pineapple2.png', 
            title: '凤梨生长过程',
            description: '从种植到采收的完整生长周期，确保每一颗凤梨都达到最佳品质'
        },
        { 
            src: 'pineapple3.png', 
            title: '成熟凤梨',
            description: '金钻凤梨果肉细腻，甜度高，香气浓郁，重量1.5-2.0kg/个'
        },
        { 
            src: 'pineapple4.png', 
            title: '丰收季节',
            description: '精心挑选，确保每一颗凤梨都达到最佳成熟度'
        },
        { 
            src: 'pineapple5.png', 
            title: '品质检测',
            description: '严格的质量检测流程，确保凤梨品质和安全'
        }
    ];
    
    const track = document.getElementById('slideshowTrack');
    const indicators = document.getElementById('slideshowIndicators');
    const infoElement = document.querySelector('.slideshow-info');
...
function addTouchSupport() {
    ...
    track.addEventListener('touchstart', (e) => {
        ...
        clearInterval(slideInterval); // 触摸时暂停自动播放
    });
...
function updateSlideInfo() {
    ...
    if (infoElement) {
        infoElement.innerHTML = `
            ...
            <span id="totalSlides">${slides.length}</span>
            <span class="slide-title-display"> - ${slides[currentSlide]?.querySelector('.slide-title')?.textContent || ''}</span>
        `;
    }
}
